...
class _ScoresScreenState extends State<ScoresScreen> with TickerProviderStateMixin {
  ...
  // Fonction pour envoyer le message de démarrage
  void _sendStartGame() {
    final webSocketService = Provider.of<WebSocketService>(context, listen: false);
    webSocketService.sendMessage(json.encode({
      'type': 'start',  // Message pour démarrer la partie
      'message': 'START_GAME',
    }));

    setState(() {
      isGameStarted = true; // La partie est lancée
    });
  }
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 8.0),
            child: ElevatedButton(
              onPressed: isGameStarted ? null : _sendStartGame, // Désactiver si la partie est lancée
              style: ElevatedButton.styleFrom(
//                backgroundColor: isGameStarted ? Colors.green : const Color(0xFF7DBFF8), // Vert si la partie est lancée
                backgroundColor: const Color(0xFF7DBFF8), // Couleur de base
                disabledBackgroundColor: Colors.green, // Couleur quand désactivé
                elevation: 0,
                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
              ),
              child: Text(
                isGameStarted ? 'Partie lancée' : 'Démarrer', // Texte dynamique
                style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.black),
              ),
            ),
          ),
  ...
  // Widget pour afficher une carte de score
