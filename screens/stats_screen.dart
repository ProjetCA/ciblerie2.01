      ...
      setState(() {
        for (int i = 0; i < 4; i++) {
          ...
          if (entries != null && entries.isNotEmpty) {
            final scores = entries.map<int>((e) {
              final score = e['score'];
              if (score is int) return score;
              if (score is double) return score.toInt(); // convertit double en int si nécessaire
              return 0;
            }).toList();
            final int total = scores.fold(0, (a, b) => a + b);
            final int best = scores.reduce((a, b) => a > b ? a : b);
            final int latest = scores.last;
            final freq = Map<int, int>.fromIterable(
              [0, 5, 10, 15, 25, 50, 100, 150, 200, 250],
              key: (e) => e as int,
              value: (_) => 0,
            );
            for (var s in scores) {
